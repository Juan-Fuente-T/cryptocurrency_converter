...
class VisualMoney(tk.Tk):
    ...
    def __init__(self):
        ...
        self.cantidad_moneda = tk.Entry(self, width=12, borderwidth=2,bg="LightSteelBlue1" )
        self.nombre_moneda_origen = ttk.Combobox(self, values=[nombre for nombre, _ in self.listado_currencies], state="readonly", width=25)
        self.nombre_moneda_destino = ttk.Combobox(self, values=[nombre for nombre, _ in self.listado_currencies], state="readonly", width=25)
...
al pulsar el botón calcular obtendrá la 
        ...
        self.nombre_moneda_origen.bind("<<ComboboxSelected>>", self.mostrar_valor_defecto)
        self.nombre_moneda_destino.bind("<<ComboboxSelected>>", self.mostrar_valor_defecto)
    ...
    def calcular_resultado(self):
        ...
        if cantidad_moneda > 1 :
            ...
            self.mostrar_valor_defecto()  # Agregado: mostrar el valor por defecto


    def mostrar_valor_defecto(self, event=None):
        if self.nombre_moneda_origen.get() and self.nombre_moneda_destino.get():
            cantidad_moneda = 1
            codigo_moneda_origen = self.diccionario_currencies[self.nombre_moneda_origen.get()]
            codigo_moneda_destino = self.diccionario_currencies[self.nombre_moneda_destino.get()]
            valor_defecto = self.calcular_divisa(cantidad_moneda, codigo_moneda_origen, codigo_moneda_destino)
